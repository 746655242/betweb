<template>
    <div class="mineWrap">
		<header class="expertsApplyHeader">
			...
			<span>我的申请</span>
			<router-link to="/myApply" class="iconfont icon-tianjia headerRight">&#xe60c;</router-link>
		</header>
					...
					<div class="applyBox"  v-for="(item,index) in applylist" :key="index">
					 ...
					 <swipeout-item  transition-mode="follow">
						...
						<div slot="content" class="demo-content vux-1px-t">
							<router-link class="expertsInfoBox" :to="'/myApply/'+item.ApplyPfUserId"  >
								<div class="experts">
									<p class="expertsName"><b>{{item.ApplyPfUserName}}</b></p>
									<p class="applyNotPass" v-if="item.ApplyPfUserStatusId==1">{{item.ApplyPfUserStatusShow}}</p>
									<p class="applyInTheReview">{{item.ApplyPfUserStatusShow}}</p>
								</div>
